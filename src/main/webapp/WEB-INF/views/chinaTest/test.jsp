...
<!DOCTYPE html>
<%
	ArrayList<WordTO> chinaList = (ArrayList<WordTO>) request.getAttribute("chinaList");
...
<body>
    ...
    <h1 style="text-align: center;">중국어 단어 테스트</h1>
  </div>	

<script>

    const words = [
        <% for (WordTO to : chinaList) { %>
    ...
    function generateWordBoxes() {
        ...
        const numberOfWordsToDisplay = <%=number%>;
    ...
    function validateForm() {
        ...
        inputElements.forEach((inputElement) => {
            ...
            if ((isKorean && inputValue !== '' && !/^[가-힣ㄱ-ㅎㅏ-ㅣ]+$/.test(inputValue)) ||
                    (!isKorean && inputValue !== '' && !/[\u4E00-\u9FFF]+/.test(inputValue))) {
                isValid = false;
            }
        });

        if (!isValid) {
            alert('단어에는 중국어만, 뜻에는 한글만 입력해야합니다!!');
        }
