...
<!DOCTYPE html>
<%
	ArrayList<WordTO> engList = (ArrayList<WordTO>) request.getAttribute("engList");
    ...
    function validateForm() {
        ...
        inputElements.forEach((inputElement) => {
            ...
            if ((isKorean && inputValue !== '' && !/^[가-힣ㄱ-ㅎㅏ-ㅣ\s]+$/.test(inputValue)) ||
                (!isKorean && inputValue !== '' && !/^[a-zA-Z\s]+$/.test(inputValue))) {
                isValid = false;
            }
        });

        if (!isValid) {
            alert('단어에는 영어만, 뜻에는 한글만 입력해야합니다!!');
        }
